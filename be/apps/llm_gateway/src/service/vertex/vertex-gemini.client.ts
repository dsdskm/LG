...
import { fetchWithTimeout, pickHeaders, safeJsonParse, truncate } from "../../utils/utils";
...
export class VertexGeminiClient {
    ...
    constructor(
        ...
        // ✅ 인증: 키 파일이 있으면 그 파일로, 없으면 환경 ADC 사용
        const trimmedKeyFile = (keyFilePath ?? "").trim();
        this.auth = new GoogleAuth({
            scopes: [authScope],
            ...(trimmedKeyFile ? { keyFile: trimmedKeyFile } : {}),
        });
    }
    ...
    async generateContent(params: VertexCallParams): Promise<VertexCallResult> {
        ...
        const body: VertexGenerateContentBody = {
            ...
            generationConfig: {
                ...
                // 순수 JSON 응답 강제 → ```json 펜스/잡설 없이 파싱 안정화
                responseMimeType: "application/json",
                // thinking 비활성 → 출력 토큰을 JSON 본문에 온전히 사용(잘림 방지)
