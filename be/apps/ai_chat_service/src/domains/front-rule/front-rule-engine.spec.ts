...
describe('ChatRuleService', () => {
  it('loads parameterized screen rules with query strings alongside app rules', async () => {
    const appRule = {
      id: 1,
      appKey: 'tms',
      screenKey: 'tms',
    ...
    const rows = await service.listByAppAndScreen(
      'tms',
      'tms/taskflows/228/canvas?source=saved',
    );
...
describe('matchFrontRuleRows', () => {
  it('matches /copy 197 and /copy 197 name when patternRegex has optional name capture', () => {
    ...
    const matched = matchFrontRuleRows({ screenKey, message: '/copy 197' }, [
      rule,
    ]);
    ...
    const namedMatched = matchFrontRuleRows(
      { screenKey, message: '/copy 197 신규 이름' },
      [rule],
    );
  ...
  it('keeps the raw matched rule data and honors navigation/replyText from extraJson', () => {
    ...
    expect(matched?.ruleData?.extraJson).toMatchObject({
      navigation: 'tms/taskflows',
    });
    expect(matched?.toolArgs).toMatchObject({
      replyText: '태스크플로우 목록으로 이동합니다.',
  ...
  it('matches an alias command from its DB rule', () => {
    ...
    expect(
      matchFrontRuleRows({ screenKey, message: 'please /undo now' }, rules),
    ).toBeNull();
  });
  ...
  it('filters rules by appKey before matching', () => {
    ...
    const matched = matchFrontRuleRows(
      { screenKey, appKey: 'robot', message: '/list' },
      [tmsRule, robotRule],
    );
  ...
  it('returns the node name capture as a rule parameter', () => {
    const rule = commandRule('node-delete', {});
    rule.patternRegex = '^!\\s*(\\S(?:.*\\S)?)\\s*$';
    rule.replyText = '$1 노드를 삭제했습니다.';

    const matched = matchFrontRuleRows({ screenKey, message: '!Joy' }, [rule]);

    expect(matched?.params).toEqual(['Joy']);
  });
  ...
  it('falls back to a valid reply text when a single taskFlowId argument is used', () => {
    ...
    const matched = matchFrontRuleRows({ screenKey, message: '/run 79' }, [
      runRule,
    ]);
  ...
  it('falls back to app-root rules when a specific screen has other rules but no match', async () => {
    const robotDetailRule = commandRule(
    ...
    const matched = await matchFrontRule(
      ...
      async (appKey, screenKey) => {
        if (screenKey === 'tms/robots/:robotId/detail')
          return [robotDetailRule];
    ...
    expect(matched?.chatActionParam).toMatchObject({
      path: 'tms/taskflows',
      app: 'tms',
    });
  });
  ...
  it('matches a taskflow graph arrow chain like Idle->Joy on the canvas route', () => {
    ...
    const matched = matchFrontRuleRows({ screenKey, message: 'Idle->Joy' }, [
      arrowRule,
    ]);
    expect(matched).not.toBeNull();
  ...
  it('matches leading-arrow canvas chains like ->Joy and ->Joy->Love', () => {
    ...
    expect(
      matchFrontRuleRows({ screenKey, message: '->Joy' }, [arrowRule]),
    ).not.toBeNull();
    expect(
      matchFrontRuleRows({ screenKey, message: '->Joy->Love' }, [arrowRule]),
    ).not.toBeNull();
    expect(
      matchFrontRuleRows({ screenKey, message: 'Joy=>Love' }, [arrowRule]),
    ).not.toBeNull();
  ...
  it('prefers the exact A->B taskflow rule over the generic graph fallback', () => {
    ...
    const matched = matchFrontRuleRows({ screenKey, message: 'A->B' }, [
      genericRule,
      exactRule,
    ]);
  ...
  it('matches taskflow management commands without robot id', () => {
    ...
    expect(
      matchFrontRuleRows({ screenKey, message: '/copy 42' }, [copyRule])
        ?.toolArgs,
    ).toMatchObject({
      type: 'copy-taskflow',
      taskFlowId: ['42'],
    ...
    expect(
      matchFrontRuleRows({ screenKey, message: '/delete 42' }, [deleteRule])
        ?.toolArgs,
    ).toMatchObject({
      type: 'delete-taskflow',
      taskFlowId: ['42'],
    ...
    expect(
      matchFrontRuleRows({ screenKey, message: '/create' }, [createRule])
        ?.toolArgs,
    ).toMatchObject({
      type: 'create-taskflow',
      replyText: '새 태스크플로우를 생성합니다.',
    ...
    expect(
      matchFrontRuleRows({ screenKey, message: '/modify 42' }, [modifyRule])
        ?.toolArgs,
    ).toMatchObject({
      type: 'modify-taskflow',
      taskFlowId: ['42'],
  ...
  it('matches deploy, run, pause, resume, and stop commands from DB rule payloads', () => {
    ...
    const deployMatched = matchFrontRuleRows(
      { screenKey, message: '/deploy robot-01 42' },
      [deployRule],
    );
    const runMatched = matchFrontRuleRows(
      { screenKey, message: '/run robot-02 7' },
      [runRule],
    );
    const pauseMatched = matchFrontRuleRows(
      { screenKey, message: '/pause robot-03 12' },
      [pauseRule],
    );
    const resumeMatched = matchFrontRuleRows(
      { screenKey, message: '/resume robot-04 13' },
      [resumeRule],
    );
    const stopMatched = matchFrontRuleRows(
      { screenKey, message: '/stop robot-05 14' },
      [stopRule],
    );
    const deploySingleArgMatched = matchFrontRuleRows(
      { screenKey, message: '/deploy 42' },
      [deployRule],
    );
    const runSingleArgMatched = matchFrontRuleRows(
      { screenKey, message: '/run AZ_U_qYSdRymNUa7uK_4' },
      [runRule],
    );
  ...
  it('keeps taskflow management and navigation commands in the SQL seed for TMS screens', () => {
    const sqlFilePath = path.resolve(
      __dirname,
      '../../../../../../sql/20260820_taskflow_management_commands.sql',
    );
